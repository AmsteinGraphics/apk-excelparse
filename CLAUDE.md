...
- **Overview page's per-group table** (`overviewTable`, built in `buildOverviewTable`): one borderless `TableRow` per group — group dots | group name | group grade (mark only, 11sp). Dots column is a **fixed width** (`maxDots × slot`) so names left-align; the name column is `setShrinkColumns(1)` so a long name wraps instead of pushing the average off-screen in portrait. Rebuilt on every overview render so live marks stay current.
