...
- **On criterion pages each graded dot shows its coefficient as a white digit** (`DotProgressView.setLabels`, `coefDigit()`), at a **fixed px size** (`DIGIT_TEXT_DP`) regardless of the dot's scale; ungraded dots show no digit. Labels are set only for the criterion-page header row — `null` on the overview row and the table.
