...
- **OVERVIEW/BACK button** (full-width, above the three nav rows): from a criterion page it stashes `pageIdx` into `overviewReturnPage` and jumps to the overview so the teacher can see the marking's effect on the general grade; there it reads **BACK** and returns to the stash. `navigate()` and `navigateGroup()` flush the stash (revert to OVERVIEW); `navigateStudent()` keeps it. Invariant: `overviewReturnPage >= 0` ⇒ currently on the overview page.
- **Nav bar is three two-button rows** stuck to the bottom, so labels don't hyphenate in portrait: `« étudiant / étudiant »`, `‹ groupe / groupe ›`, `‹ critère / critère ›`.
...
- **Dots scale by coefficient** (`coefScale`): coef 2 = 100%, ±25% per integer unit (`1 + (coef−2)·0.25`), clamped `[0.4, 2.0]` and radius capped at the row height. Bigger dot = heavier criterion.
- **`DotProgressView.setFixedSlotPx`**: forces a fixed per-dot slot width so several instances render dots at an identical physical size regardless of their own width. The overview page's per-group table uses this with `headerSlotPx()` = `(screenWidth − 48dp) / criteria.size()` and a 22dp row height so each group's dots exactly match the full-width header string.
- **Overview page's per-group table** (`overviewTable`, built in `buildOverviewTable`): one borderless `TableRow` per group — group dots | group name | group grade (mark only). Rebuilt on every overview render so live marks stay current.
