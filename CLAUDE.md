...
- **Paging = one overview page per student + one page per criterion.** `MainActivity.pageIdx` runs `0..criteria.size()`; page 0 is the read-only **overview** (general CL average + dots for *all* criteria, slider/criterion detail hidden), pages `1..N` are criterion pages (criterion index = `pageIdx - 1`). A criterion page scopes the dot row and the top-right average to the criterion's **group** (group's dots only; group's "sur 6" grade). `navigate()` flattens over `students × (criteria + 1)`; `navigateStudent()` keeps the same page index across students. In-memory `pageIdx`/`studentIdx` survive rotation via the same `configChanges` mechanism (not persisted to `savedInstanceState`).
