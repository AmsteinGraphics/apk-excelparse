...
- **Group filter (burger menu → "filtrer les groupes").** A checkbox list of groups (all ticked by default) lets a teacher hide the groups they don't evaluate. `groupHidden[]` (indexed by group index) drives visibility everywhere: overview table rows, the overview dot row (`visibleCriterionIndices()` — non-contiguous safe), `navigateGroup` (cycles visible groups only), and the criterion pages. **Never touches the workbook's formulas** — it's pure display/navigation. Persisted **per workbook** via `putStringSet(FILTER_PREFIX + displayName, hiddenGroupNames)`; a newly picked file starts all-ticked. The filter UI **forbids unticking the last visible group**. On leaving the filter, if the current criterion page sits in a now-hidden group it falls back to the overview.
